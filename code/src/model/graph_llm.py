...
class GraphLLM(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if args.llm_model_name == "7b" or args.llm_model_name == "7b_chat":
            outputsize = 4096
        elif args.llm_model_name == "13b" or args.llm_model_name == "13b_chat":
            outputsize = 5120
    ...
    def maybe_autocast(self, dtype=torch.float16, inference = False):
        # if on cpu, don't use autocast
        # if on gpu, use autocast with dtype if provided, otherwise use torch.float16
        enable_autocast = self.device != torch.device("cpu")

        if enable_autocast and not inference:
    ...
    def inference_sub(self, sample):

        # Make a "batch" with a single sample
        samples = {
            "id": [sample["id"]],
        ...
        # Encode + project graph
        graphs = samples["graph"][0].to(self.device)
        n_embeds, _ = self.graph_encoder(graphs.x, graphs.edge_index.long(), graphs.edge_attr)
        graph_embeds = n_embeds.mean(dim=0)  # [hidden_dim]
        graph_embeds = self.projector(graph_embeds)
        graph_embeds = graph_embeds.unsqueeze(0)  # Add [1, hidden_dim] dim to simulate batch of 1

        # Since we "batched" it manually
        batch_size = 1
        batch_inputs_embeds = []
        batch_attention_mask = []
        ...
        for i in range(batch_size):
            ...
            # Concatenate bos + graph + text embeddings
            inputs_embeds = torch.cat([bos_embeds, graph_embeds, inputs_embeds], dim=0)

            batch_inputs_embeds.append(inputs_embeds)
            batch_attention_mask.append([1] * inputs_embeds.shape[0])

        # Pad for batch dim = 1 (same as normal batching)
        max_length = max([x.shape[0] for x in batch_inputs_embeds])
        for i in range(batch_size):
            pad_length = max_length - batch_inputs_embeds[i].shape[0]
            batch_inputs_embeds[i] = torch.cat([pad_embeds.repeat(pad_length, 1), batch_inputs_embeds[i]])
